// Implementation of System Calls: cps130 and Process Managment Functions
...
cps130() {
    ...
    // search in the process table for process with pid
    acquire(&ptable.lock);
    ...
    for (p = ptable.proc; p < &ptable.proc[NPROC]; p++) {
        ppid = (p->pids[0].pid == 1 && p->pids[1].pid == 0) ? 0
                                                            : p->parent->ns_pid; // for `init` process - ppid = 0
