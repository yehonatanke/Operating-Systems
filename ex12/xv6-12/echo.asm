...
main(int argc, char *argv[])
{
  ...
  if (argc > 1 && !strcmp("-n", argv[1])) {
  1f:	0f 8e f1 00 00 00    	jle    116 <main+0x116>
  25:	89 c6                	mov    %eax,%esi
  27:	50                   	push   %eax
  28:	50                   	push   %eax
  29:	ff 77 04             	pushl  0x4(%edi)
  2c:	68 28 0c 00 00       	push   $0xc28
  31:	e8 5a 01 00 00       	call   190 <strcmp>
  36:	83 c4 10             	add    $0x10,%esp
  39:	85 c0                	test   %eax,%eax
  3b:	0f 84 c2 00 00 00    	je     103 <main+0x103>
  int i = 1;
  int j = 0;
  int size = 0;
  char * data = 0;
  int offset = 0;
  char new_line = 1;
  41:	c6 45 db 01          	movb   $0x1,-0x25(%ebp)
 */
...
main(int argc, char *argv[])
{
  ...
  5e:	83 c3 01             	add    $0x1,%ebx
    size += strlen(argv[j]) + 1;
  61:	e8 0a 02 00 00       	call   270 <strlen>
  if (argc > 1 && !strcmp("-n", argv[1])) {
    ++i;
    new_line = 0;
  }
  ...
  69:	39 5d e0             	cmp    %ebx,-0x20(%ebp)
    size += strlen(argv[j]) + 1;
  6c:	8d 74 30 01          	lea    0x1(%eax,%esi,1),%esi
  if (argc > 1 && !strcmp("-n", argv[1])) {
    ++i;
    new_line = 0;
  }
  ...
  77:	8b 75 e4             	mov    -0x1c(%ebp),%esi
  7a:	52                   	push   %edx
  7b:	e8 70 09 00 00       	call   9f0 <malloc>
  ...
  data = malloc(size);
  85:	89 45 e4             	mov    %eax,-0x1c(%ebp)

  if (!data) {
  88:	0f 84 aa 00 00 00    	je     138 <main+0x138>
  8e:	31 db                	xor    %ebx,%ebx
  90:	eb 10                	jmp    a2 <main+0xa2>
  ...
  9b:	c6 04 18 20          	movb   $0x20,(%eax,%ebx,1)
      ++offset;
  ...
  a5:	ff 34 b7             	pushl  (%edi,%esi,4)
  a8:	e8 c3 01 00 00       	call   270 <strlen>
    memmove(data + offset, argv[i], length);
  ad:	83 c4 0c             	add    $0xc,%esp
  b0:	89 45 dc             	mov    %eax,-0x24(%ebp)
  b3:	50                   	push   %eax
  b4:	8b 45 e4             	mov    -0x1c(%ebp),%eax
  b7:	ff 34 b7             	pushl  (%edi,%esi,4)
    offset += length;
    if (i + 1 < argc) {
  ...
  for (; i < argc; ++i) {
    int length = strlen(argv[i]);
    memmove(data + offset, argv[i], length);
  bd:	01 d8                	add    %ebx,%eax
  bf:	50                   	push   %eax
  c0:	e8 2b 03 00 00       	call   3f0 <memmove>
    offset += length;
  ...
  for (; i < argc; ++i) {
    ...
    memmove(data + offset, argv[i], length);
    offset += length;
  cb:	01 d3                	add    %edx,%ebx
    if (i + 1 < argc) {
  cd:	39 75 e0             	cmp    %esi,-0x20(%ebp)
  d0:	7f c6                	jg     98 <main+0x98>
      data[offset] = ' ';
      ++offset;
    }
  }
  ...
  if (new_line) {
  d2:	80 7d db 00          	cmpb   $0x0,-0x25(%ebp)
  d6:	75 54                	jne    12c <main+0x12c>
    data[offset] = '\n';
    ++offset;
  }

  result = write(1, data, offset);
  d8:	8b 7d e4             	mov    -0x1c(%ebp),%edi
  db:	83 ec 04             	sub    $0x4,%esp
  de:	53                   	push   %ebx
  df:	57                   	push   %edi
  e0:	6a 01                	push   $0x1
  e2:	e8 31 05 00 00       	call   618 <write>
  e7:	89 c3                	mov    %eax,%ebx
  free(data);
  e9:	89 3c 24             	mov    %edi,(%esp)
  ec:	e8 6f 08 00 00       	call   960 <free>

  if (-1 == result) {
  f1:	83 c4 10             	add    $0x10,%esp
  f4:	83 fb ff             	cmp    $0xffffffff,%ebx
  f7:	74 3f                	je     138 <main+0x138>
    exit(1);
  }
  ...
  fc:	6a 00                	push   $0x0
  fe:	e8 f5 04 00 00       	call   5f8 <exit>
  if (argc > 1 && !strcmp("-n", argv[1])) {
    ++i;
    new_line = 0;
  }
  ...
  for (j = i; j < argc; ++j) {
 103:	83 fe 02             	cmp    $0x2,%esi
 106:	74 3a                	je     142 <main+0x142>
  char new_line = 1;
  int result = 0;
  ...
  if (argc > 1 && !strcmp("-n", argv[1])) {
    ++i;
    new_line = 0;
 108:	c6 45 db 00          	movb   $0x0,-0x25(%ebp)
  int offset = 0;
  char new_line = 1;
  int result = 0;
  ...
  if (argc > 1 && !strcmp("-n", argv[1])) {
    ++i;
 10c:	be 02 00 00 00       	mov    $0x2,%esi
 111:	e9 34 ff ff ff       	jmp    4a <main+0x4a>
 ...
 11b:	6a 00                	push   $0x0
 11d:	e8 ce 08 00 00       	call   9f0 <malloc>
 ...
 125:	85 c0                	test   %eax,%eax
  ...
  data = malloc(size);
 127:	89 45 e4             	mov    %eax,-0x1c(%ebp)

  if (!data) {
 12a:	74 0c                	je     138 <main+0x138>
      ++offset;
    }
  }
 ...
 12f:	c6 04 18 0a          	movb   $0xa,(%eax,%ebx,1)
    ++offset;
 133:	83 c3 01             	add    $0x1,%ebx
 136:	eb a0                	jmp    d8 <main+0xd8>
  }
 ...
 13b:	6a 01                	push   $0x1
 13d:	e8 b6 04 00 00       	call   5f8 <exit>
 ...
 145:	6a 00                	push   $0x0
 147:	e8 a4 08 00 00       	call   9f0 <malloc>
 ...
 151:	89 45 e4             	mov    %eax,-0x1c(%ebp)
  ...
  if (!data) {
 154:	74 e2                	je     138 <main+0x138>
 156:	31 db                	xor    %ebx,%ebx
 158:	e9 7b ff ff ff       	jmp    d8 <main+0xd8>
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 737:	f7 f1                	div    %ecx
 739:	0f b6 92 34 0c 00 00 	movzbl 0xc34(%edx),%edx
  }while((x /= base) != 0);
...
printstr(int fd, char *str)
{
  ...
  if (str == 0)
    str = "(null)";
 903:	b8 2b 0c 00 00       	mov    $0xc2b,%eax
